...
#include <linux/proc_fs.h>       /* work with /proc */
...
/* function to manage the information in /proc */
                   ...
                   int count, int *eof, void *data)
{
    ...
    //len += sprintf(buf+len,"\nDevice name:\t%s \n Major number:\t%d\n Minor number:\t%d","HELLO_LEO",MAJOR(),MINOR() );
    len += sprintf(buf+len,"\nDevice \n CIAO LEO");
    up(&(hello_devices->sem_hello));
    *eof = 1;
    return len;
...
static int hello_init(void)
{
    ...
    proc_file_entry = proc_create("LEO_hello", 0, NULL, &hello_fops);
...
static void hello_exit(void)
{
	  ...
	  hello_cleanup_module();
    remove_proc_entry("LEO_hello", NULL /* parent dir */);
}
