 ...
 * */
 ...
 static int LED_of_probe(struct platform_device *pdev)
 {
     int irq = -1;
     int ret;

 	   LED_01_devices->temp_res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
 	   if (!(LED_01_devices->temp_res)) {
 	   	dev_err(&pdev->dev, "TRY could not get IO memory\n");
 	   	return -ENXIO;
 	   }

           printk("TRY resource : regs.start=%#x,regs.end=%#x\n",LED_01_devices->temp_res->start,LED_01_devices->temp_res->end);
     ...
     /* What I normally do */
     ret = of_address_to_resource(pdev->dev.of_node, 0, &(LED_01_devices->res));
     if (ret) {
     ...
     LED_01_devices->mem_region_requested = request_mem_region((LED_01_devices->temp_res->start),resource_size(LED_01_devices->temp_res),"LED_01");
 ...
 {
     release_mem_region((LED_01_devices->temp_res->start),resource_size(LED_01_devices->temp_res));
  ...
  */
  ...
  {
    LED_01_devices->addr_tmp = ioremap((LED_01_devices->temp_res->start),resource_size(LED_01_devices->temp_res));
    iowrite32(LED_DIRECTION_OUTPUT,LED_01_devices->addr_tmp + XGPIO_TRI_OFFSET); /* Set output directio */
    iowrite32(LED_01_devices->LED_value , LED_01_devices->addr_tmp); /* Set output LED 1 and 4 (binary of decimal 8) */
    iounmap(LED_01_devices->addr_tmp);
