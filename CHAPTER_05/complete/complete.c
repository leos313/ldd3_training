/*
 *
 * The original file was modified in order to test it with a real code (also
 * provided as a source file).
 * Following the original incipt of the original source code
 *
...
{
	printk(KERN_DEBUG "[LEO] process %i (%s) awakening the readers...\n", current->pid, current->comm);
	complete(&comp);
...
{
  printk(KERN_INFO "[LEO] performing 'complete' release\n");
    return 0;
}
